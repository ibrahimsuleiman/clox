...
static struct entry *find_entry(struct entry *entries, int capacity, obj_string_t *key)
{
...
bool table_set(struct table *t, obj_string_t *key, value_t value)
{
        ...
        if(is_new_key && IS_NIL(e->value)) t->count++; /* only increment where there are no tombstones*/
...
{
        if(t->count == 0) return false; /* empty? */
...
bool table_delete(struct table *t, obj_string_t *key)
{
        ...
        if(entry->key == NULL) return false; /* no such entry*/
